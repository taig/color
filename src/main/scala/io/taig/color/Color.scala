...
object Color {
  ...
  def fromHex(value: Long, digits: Int): Either[String, Color] =
    if (value < 0x0 || value > 0XFFFFFFFFL)
      ...
      val red = Channel.unsafe(((value & 0xFFF) >> 8) * 17)
      val green = Channel.unsafe(((value & 0xFF) >> 4) * 17)
      val blue = Channel.unsafe((value & 0xF) * 17)
      Right(Color(red, green, blue, None))
    } else if (digits == 4) {
      val red = Channel.unsafe(((value & 0xFFFF) >> 12) * 17)
      val green = Channel.unsafe(((value & 0xFFF) >> 8) * 17)
      val blue = Channel.unsafe(((value & 0xFF) >> 4) * 17)
      val alpha = Channel.unsafe((value & 0xF) * 17)
      Right(Color(red, green, blue, Some(alpha)))
    } else if (digits == 6) {
      val red = Channel.unsafe((value & 0xFFFFFF) >> 16)
      val green = Channel.unsafe((value & 0xFFFF) >> 8)
      val blue = Channel.unsafe(value & 0xFF)
      Right(Color(red, green, blue, None))
    } else if (digits == 8) {
      val red = Channel.unsafe((value & 0xFFFFFFFF) >> 24)
      val green = Channel.unsafe((value & 0xFFFFFF) >> 16)
      val blue = Channel.unsafe((value & 0xFFFF) >> 8)
      val alpha = Channel.unsafe(value & 0xFF)
      Right(Color(red, green, blue, Some(alpha)))
    } else {
